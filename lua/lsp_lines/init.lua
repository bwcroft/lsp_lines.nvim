...
---@param opts boolean
local function init(namespace, bufnr, diagnostics, opts)
  vim.validate({
  ...
  for lnum, lelements in pairs(line_stacks) do
    ...
    if buf_count ~= 0 or lnum <= buf_count then
      vim.api.nvim_buf_set_extmark(bufnr, virt_lines_ns, lnum, 0, {
        id = lnum + 1, -- Must be positive; +1 covers line=0.
        virt_lines = virt_lines,
        virt_lines_above = lnum == buf_count - 1, -- Apply above buffer if last line
      })
    end
  end
end
